...
overall_accuracy_train<-double(accuracy_size)
overall_accuracy_test<-double(accuracy_size)
time_train<-double(accuracy_size)
time_test<-double(accuracy_size)
w<-character(accuracy_size)
...
# save(nondata_II, file="nondata_II.saved")
##########################################################

load("nodata.saved")
sampling_I<-sample(nrow(nodata),size_training_set)
train_x_corpus<-nodata[sampling_I,]
total<-c(1:len_stemmed_article)
test_x_corpus<-nodata[sample(total[!total %in% sampling_I],max_to_take),]
...
sampling_II<-sample(nrow(nondata),size_training_set)
train_x_non_corpus<-nondata[sampling_II,]
test_x_non_corpus<-nondata[sample(total_II[!total_II %in% sampling_II],max_to_take),]


for (iterate_time in 1:accuracy_size) 
{
  # iterate_time=accuracy_size-iterate_time+1
  #iterate_time=100
...
#   summ<-summary(obj)
  ...
  accuracy
  overall_accuracy_train[iterate_time]<-accuracy
  ...
  accuracy
  overall_accuracy_test[iterate_time]<-accuracy
  
  w[iterate_time] = t(model$coefs) %*% model$SV

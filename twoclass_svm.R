...
nodata<-subset(nodata, select = c(words$Word,"result"))
train_x_corpus<-nodata[c(1:size_training_set),]
test_x_corpus<-nodata[c((size_training_set+1):len_stemmed_article),]
...
train_x_non_corpus<-nondata[c(1:size_training_set),]
test_x_non_corpus<-nondata[c((size_training_set+1):len_non_corpus_stemmed_articles),]
...
main_frame<-as.data.frame(cbind(overall_accuracy_test,x_input))

test_data_long <- melt(main_frame, id="x_input")  # convert to long format

ggplot(data=main_frame,
       aes(x=x_input, y=overall_accuracy_test, colour="red")) +
  geom_line()

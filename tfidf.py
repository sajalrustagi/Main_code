...
#    time.sleep(0.1)
#    if tf_value>10 :
#        return str()+","+str(tf_value)+","+str(df_value)
    

list_keys=dictionary.token2id.keys()
tf_df_values = pool.map(extract_tf_df, range(len(list_keys)))

values_tf={}
values_df={}
for indice in range(len(list_keys)):
    print indice
    key=list_keys[indice]
    value=dictionary.token2id[key]
    tf_value=0
    df_value=0
    for items in corpus :
        for tuples in items :
            if tuples[0]==value:
                ...
                df_value=df_value+1
    if tf_value>=10 :
        values_tf[key]=tf_value
        values_df[key]=df_value

...
def stem_stop_token(index):
    ...
    update_sql="UPDATE `Articles_English_language_Non_corpus_II`  SET Stemmed_Article=\""+ str(stemmed_big_tokens)[1:-1]+"\" where ArticleText=\""+i+"\""
    #print update_sql
#    try :
...
sql="SELECT distinct ArticleText FROM `Articles_English_language_Non_corpus_II`  where Stemmed_Article=\"\""
doc_set=[]
try :
    cursor.execute(sql)
